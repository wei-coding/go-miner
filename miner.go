...
var getPoolAddr string = "https://server.duinocoin.com/getPool"
...
func work() {
    ...
    for {
        ...
        // Requesting a job.
        _,
        err = conn.Write([] byte("JOB," + username + "," + diff + "," + key + "," + temp + ","))
        ...
        _,
        err = conn.Read(buffer) // Getting the jobs.
        ...
        for i: = 0;i <= diff * 100;i++{
            ...
            if nh == goal {
                ...
                _,
                err = conn.Write([] byte(strconv.Itoa(i) + "," + strconv.FormatFloat(hash_rate, 'f', 2, 64) + ",Go Miner on RPi,Go Miner"))
                ...
                _,
                err = conn.Read(feedback_buffer) // Reads response.
...
func main() {
    ...
    if s.Success != true {
        log.Println("Failed when getting pool info.")
        return
    }

    addr = s.Ip + ":" + strconv.Itoa(s.Port)
    fmt.Println("Pool's IP " + addr)
